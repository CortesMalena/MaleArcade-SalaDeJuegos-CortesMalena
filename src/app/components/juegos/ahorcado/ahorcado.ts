...
export class Ahorcado implements OnInit, OnDestroy {
  ...
  async obtenerPalabra() {
    ...
    try {
      ...
      if (data) {
        ...
        this.tiempo.set(0);
        this.pista.set(data.pista);
        this.estadistica.palabra_id = data.id ?? 0;
  ...
  actualizarNumImg() {
    this.loading.set(true);
    const rutaBaseImg = '../../../../assets/ahorcado/ahorcado';
    this.numImg++;
    this.imagen.set(`${rutaBaseImg}${this.numImg}.png`);
    this.loading.set(false);
  }
  ...
  async finalizarJuego(victoria: boolean) {
    ...
    try {
      ...
      if (usuarioData) {
        ...
        this.estadistica.palabra = this.palabraAAdivinar
        this.estadistica.letras_seleccionadas = this.letrasSeleccionadas();
